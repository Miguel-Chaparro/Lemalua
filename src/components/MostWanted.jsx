...
const wantedProducts = [
  ...
  },
  {
    ...
    description: 'Destilado 5 veces | Rusia • Filtrado en cristal volcánico para una pureza excepcional.',
  ...
  },
  {
    ...
    rating: 4,
    category: 'Licor',
    description: 'Café de origen colombiano | 20% ABV • Notas dulces y ahumadas con cuerpo aterciopelado.',
...
export default function MostWanted({ onSelectProduct }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter">
          {wantedProducts.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="text-center space-y-4 luxury-card-hover p-4 rounded-lg bg-surface-container-low/30 border border-outline-variant/10 flex flex-col justify-between group cursor-pointer relative overflow-hidden"
              ...
              >
                {/* In-Cart indicator */}
                {inCart && (
                  <span className="absolute top-3 right-3 z-10 bg-secondary text-primary-container w-6 h-6 rounded-full flex items-center justify-center shadow-lg animate-[pulse_2s_ease-in-out_infinite]">
                    <span className="material-symbols-outlined text-xs" style={{ fontVariationSettings: "'FILL' 1" }}>check</span>
                  </span>
                )}

                {/* Hover overlay */}
                <div className="absolute inset-0 bg-secondary/0 group-hover:bg-secondary/3 transition-colors duration-300 pointer-events-none rounded-lg" />

                <div>
                  <div className="aspect-square bg-surface flex items-center justify-center p-8 overflow-hidden rounded-md relative">
                    <img
                      alt={product.name}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-background/40">
                      <span className="material-symbols-outlined text-secondary text-2xl">zoom_in</span>
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-on-surface-variant/60 mt-1 line-clamp-2 px-2">{product.description}</p>
                  <p className="text-secondary font-label-md mt-2">${product.price.toFixed(2)}</p>
                </div>
                ...
                <div className="pt-2" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    className={`w-full py-2 border font-label-sm uppercase tracking-wider transition-all duration-300 rounded active:scale-95 flex items-center justify-center gap-2 ${
                      inCart
                        ? 'border-secondary bg-secondary/10 text-secondary'
                        : 'border-secondary/50 text-secondary hover:bg-secondary hover:text-primary-container bg-transparent'
                    }`}
                  >
