...
export default function Catalog({ onSelectProduct }) {
  ...
  return (
    <>
      {/* Hero Section / Header */}
      <header className="px-4 md:px-margin-desktop max-w-container-max mx-auto py-12 md:py-20 border-b border-outline-variant/10">
      ...
      <div className="lg:hidden sticky top-[68px] z-30 bg-background/95 backdrop-blur-md border-b border-outline-variant/15 shadow-lg shadow-background/50">
        ...
        >
          {categories.map((cat) => {
            const isHighlighted = activeCategory === cat.id.toString();
            return (
              <button
                key={cat.id}
                data-cat-id={cat.id}
        ...
        <div className="flex-grow">
          ...
          {searchQuery ? (
            ...
            categories.map(category => {
              ...
              const isBento = category.nombre.toLowerCase().includes('temporada') || category.nombre.toLowerCase().includes('oferta') || category.nombre.toLowerCase().includes('rara');
