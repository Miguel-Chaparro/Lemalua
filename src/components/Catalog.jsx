              ...
              {categories.map((cat, index) => {
                ...
                return (
                  <li key={cat.id}>
                    <a
                      onClick={(e) => handleCategoryClick(cat.id, e)}
                      className={`font-headline-sm text-headline-sm hover:text-secondary transition-colors flex items-center justify-between group cursor-pointer ${isActive ? 'text-secondary font-medium' : 'text-on-surface-variant'
                        }`}
                      href={`#section-${cat.id}`}
                    >
                      {cat.label}
                      <span
                        className={`text-label-sm transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'
                          }`}
                      >
                    ...
                    <div className="relative bg-surface-container aspect-[3/4] overflow-hidden mb-4 border border-outline-variant/10 group-hover:border-secondary/30 transition-all duration-500">
                      ...
                      <div className="quick-add absolute bottom-0 left-0 w-full p-4 opacity-0 transform translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                        <button
                          ...
                          className={`w-full py-3 border text-secondary font-label-md uppercase tracking-widest transition-colors duration-300 flex items-center justify-center gap-2 ${isAdded
